const RecursoRevisionSegundoPeriodo = require("../../models/secretaria-consejo/RecursoRevisionSegundoPeriodo");

const obtenerRecursos = async (req, res) => {
    try {
        const recursos = await RecursoRevisionSegundoPeriodo.find().sort({ createdAt: -1 });
...
const crearRecurso = async (req, res) => {
    try {
        ...
        const nuevoRecurso = new RecursoRevisionSegundoPeriodo({
            titulo,
            descripcion,
            nombreArchivo,
            archivoBase64,
...
const actualizarRecurso = async (req, res) => {
    try {
        const { id } = req.params;
        const actualizado = await RecursoRevisionSegundoPeriodo.findByIdAndUpdate(id, req.body, { new: true });
...
const eliminarRecurso = async (req, res) => {
    try {
        const { id } = req.params;
        await RecursoRevisionSegundoPeriodo.findByIdAndDelete(id);
