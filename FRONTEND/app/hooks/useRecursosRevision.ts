...
export function useRecursosRevision(
  tipoSesion: string = "recurso-segundo-periodo",
) {
  ...
  const cargarArchivos = useCallback(async () => {
    ...
    setCargando(true);
    try {
      const datos = await fetchSesionesRevisadora(tipoSesion);
  ...
  const iniciarEdicion = (sesion: SesionDocumento) => {
    setEditandoId(sesion._id);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (editandoId) {
        ...
        const registroActualizado = await actualizarSesionRevisadora(
          editandoId,
          payloadActualizado,
        );
        if (!montadoRef.current) return;
      ...
      setNotificacion({
        tipo: "error",
        mensaje: error.message || "Ocurrió un error al procesar el archivo.",
      });
    } finally {
      if (montadoRef.current) setSubiendo(false);
    }
  }, [titulo, descripcion, archivo, editandoId, tipoSesion]);
  ...
  const handleEliminar = useCallback(async (id: string) => {
    try {
      ...
      setNotificacion({
        tipo: "error",
        mensaje: "No se pudo eliminar el registro del servidor.",
      });
    }
  }, []);
