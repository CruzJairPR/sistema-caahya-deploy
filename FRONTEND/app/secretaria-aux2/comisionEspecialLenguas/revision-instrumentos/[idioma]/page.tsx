...
export default function RevisionInstrumentosPage() {
  ...
  // Asistente Ejecutiva es solo consulta: no puede subir, editar ni eliminar registros de revisión
  const esSoloConsulta = rolUsuario === "ASISTENTE_EJECUTIVA";

  // Endpoint ajustado para la nueva API/Backend
  const endpointDinamico = `revision-instrumentos/${idiomaRaw}/${
    tipoExamenTab === "dominio" ? "examen-de-dominio" : "examen-de-metodologia"
  }`;

  // Extraemos también comentarios y fechaArchivo del hook
  const {
    titulo,
    setTitulo,
    descripcion,
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "#f5f5f5" }}>
      ...
      >
        <BannerInstitucional
          titulo={`Revisión de instrumentos - ${idiomaFormateado}`}
        />
        ...
        >
          <Typography variant="h6" sx={{ fontWeight: 600, color: "#333" }}>
            {tipoExamenTab === "dominio"
              ? `Revisión de Dominio (${idiomaFormateado})`
              : `Revisión de Metodología (${idiomaFormateado})`}
          </Typography>
        ...
        >
          {cargando ? (
            ...
            <MediaTable
              ...
              labelColumnaArchivo="Documento en Revisión"
      ...
      {!esSoloConsulta && (
        ...
        >
          ...
          >
            {editandoId
              ? "Editar Instrumento"
              : "Subir Instrumento para Revisión"}
            <IconButton
